...
// [[Rcpp::export]]
IntegerVector fh_meanshift_cpp(NumericVector img_d,
  ...
  if(pca_dim < 1) pca_dim = 1;
  if(pca_dim > nb) pca_dim = nb;
  ...
  // -----------------------------
  // B) Region stats (mean spectra + centroid)
  // -----------------------------
  std::vector<double> sumSpec((size_t)R * nb, 0.0);
  ...
  for(int r=0;r<nrow;++r){
    for(int c=0;c<ncol;++c){
      ...
      sumCol[rid] += (double)c;
      for(int k=0;k<nb;++k){
        sumSpec[(size_t)rid*nb + k] += (double)img[p + npix*k];
      }
    }
  }

  arma::mat MeanSpec(R, nb);     // renamed from M
  arma::vec wR(R);
  arma::mat Cxy(R, 2);

  for(int i=0;i<R;++i){
    double inv = 1.0 / std::max(1, rSize[i]);
    for(int k=0;k<nb;++k){
      MeanSpec(i,k) = sumSpec[(size_t)i*nb + k] * inv;
    }
    Cxy(i,0) = sumRow[i] * inv;
    Cxy(i,1) = sumCol[i] * inv;
    wR(i) = (double)rSize[i];
  }

  // Weighted PCA of region means
  arma::rowvec mu = arma::sum(MeanSpec.each_col() % wR, 0) / arma::accu(wR);
  arma::mat X = MeanSpec.each_row() - mu;

  arma::mat Cov(nb, nb, arma::fill::zeros);
  double wsum = arma::accu(wR);
  if(wsum <= 0) wsum = 1.0;

  for(int i=0;i<R;++i){
    arma::rowvec xi = X.row(i);
    Cov += (wR(i) / wsum) * (xi.t() * xi);
  }

  arma::vec eigval;
  arma::mat eigvec;
  arma::eig_sym(eigval, eigvec, Cov); // ascending
  arma::mat P = eigvec.cols(nb - pca_dim, nb - 1); // nb x pca_dim
  arma::mat PCs = X * P; // R x pca_dim

  // Build normalized feature z = [PC/ms_ranger, row/ms_hs, col/ms_hs]
  const int dim = pca_dim + 2;
  ...
  for(int i=0;i<R;++i){
    for(int d=0; d<pca_dim; ++d){
      z[(size_t)i*dim + d] = (float)PCs(i,d) * invRng;
    }
    z[(size_t)i*dim + pca_dim + 0] = (float)Cxy(i,0) * invHs;
    z[(size_t)i*dim + pca_dim + 1] = (float)Cxy(i,1) * invHs;
  }
    ...
    for(int it=0; it<ms_max_iter; ++it){
      ...
      for(const auto& off : neigh){
        ...
        for(int j : itg->second){
          ...
          if(d2 > 9.0f) continue; // 3-sigma cut
  ...
  // mode grid uses floor(y / mode_merge)
  ...
  // -----------------------------
  // E) Region adjacency from FH labels
  // -----------------------------
  ...
  // Build edges between modes with a cheap distance proxy and sort
